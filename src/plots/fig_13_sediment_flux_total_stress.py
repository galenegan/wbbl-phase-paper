...
tau_mean_vec = np.nanmean(data["tau_wc_maj"][zavg_tau, :, :][:, posidx, :], axis=(0, 1))
tau_mean_vec_plot = np.concatenate((tau_mean_vec, [tau_mean_vec[0]]))
# %% Plotting
P, Z = np.meshgrid(phase_plot, 100 * z_vec)
fig, ((ax1, ax4), (ax2, ax5), (ax3, ax6)) = plt.subplots(3, 2, sharex=True, figsize=(21, 16), layout="constrained")
ax1.plot(phase_plot, tau_mean_vec_plot, "-", linewidth=2.5, color="0.0")
ax1.set_ylabel(r"$\langle \tau_{x} \rangle$ (Pa)")
...
ax2.set_ylabel(r"$\tilde{u}_p$ (m s$^{-1}$)")
ax22.set_ylabel(r"$\partial_t \tilde{u}_p$ (m s$^{-2}$)", color="0.4")
[t.set_color("0.4") for t in ax22.yaxis.get_ticklines()]
[t.set_color("0.4") for t in ax22.yaxis.get_ticklabels()]
ax2.set_title("(b)")
ax2.set_ylim(-0.06, 0.06)
ax22.set_ylim(-0.15, 0.15)

levels = np.linspace(-2.1e-4, 2.1e-4, 17)
pc = ax3.contourf(P, Z, cpwp_plot, vmin=-2.1e-4, vmax=2.1e-4, levels=levels, cmap="RdBu")
cb = fig.colorbar(pc, ax=ax3, pad=-0.2)
pc.set_clim(-2.1e-4, 2.1e-4)
...
cb.ax.yaxis.set_ticks([-2e-4, -1e-4, 0, 1e-4, 2e-4])

ax3.plot(phase_plot, 0.4 * np.ones(len(phase_plot)), "--", linewidth=2, color="0.4")
...
tau_mean_vec = np.nanmean(data["tau_wc_maj"][zavg_tau, :, :][:, negidx, :], axis=(0, 1))
tau_mean_vec_plot = np.concatenate((tau_mean_vec, [tau_mean_vec[0]]))
# %% Plotting
ax4.plot(phase_plot, tau_mean_vec_plot, "-", linewidth=2.5, color="0.0")
ax4.set_ylabel(r"$\langle \tau_{x} \rangle$ (Pa)")
...
ax5.set_ylabel(r"$\tilde{u}_p$ (m s$^{-1}$)")
ax52.set_ylabel(r"$\partial_t \tilde{u}_p$ (m s$^{-2}$)", color="0.4")
...
ax52.set_ylim(-0.15, 0.15)

levels = np.linspace(-2.1e-4, 2.1e-4, 17)
pc = ax6.contourf(P, Z, cpwp_plot, levels=levels, cmap="RdBu")
cb = fig.colorbar(pc, ax=ax6, pad=-0.25)
# pc.set_clim(-1.75e-4, 1.75e-4)
cb.set_label(r"$\overline{c^\prime w^\prime}$ (g m$^{-2}$ s$^{-1}$)")
...
# Dashed line at canopy height
ax6.plot(phase_plot, 0.4 * np.ones(len(phase_plot)), "--", linewidth=2, color="0.4")
...
plt.savefig("files/sediment_flux.png", dpi=300)
plt.show()
